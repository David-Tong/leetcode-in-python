class Solution(object):
    def minRefuelStops(self, target, startFuel, stations):
        ...
        # dp[x][y] - the max remain fuel at xth stations after y times fuel
        dp = [[-1] * (L - 1) for _ in range(L)]
        dp[0][0] = startFuel
        ...
        for x in range(1, L):
            for y in range(x):
                if y < L - 1:
                    for z in range(x):
                        remain = dp[z][y] - (stations[x][0] - stations[z][0])
                        if remain >= 0:
                            dp[x][y] = max(dp[x][y], remain)
                            if y < L - 2:
                                dp[x][y + 1] = max(dp[x][y], remain + stations[x][1])
