class MovieRentingSystem(object):
    ...
    def __init__(self, n, entries):
        ...
        self.dicts = defaultdict(str)
        self.avails = defaultdict(bool)
        self.movies = defaultdict(list)
        self.records = list()
        for entry in entries:
            shop, movie, price = entry
            key = "{}-{}".format(shop, movie)
            label = "{}-{}-{}".format
            self.dicts[label] = key
            self.avails[key] = True
            self.movies[movie].append((price, shop, movie))
            self.records.append((price, shop, movie))

        # sort
        for movie in self.movies:
            self.movies[movie] = sorted(self.movies[movie])
    ...
    def search(self, movie):
        ...
        res = list()
        idx = 0
        for record in self.movies[movie]:
            price, shop, _ = record
            key = "{}-{}".format(shop, movie)
            if self.avails[key]:
                res.append(shop)
    ...
    def report(self):
        ...
        for record in self.records:
            price, shop, movie = record
            key = "{}-{}".format(shop, movie)
            if not self.avails[key]:
