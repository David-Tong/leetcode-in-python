class Solution(object):
    def countPaths(self, n, roads):
        ...
        for road in roads:
            dicts[road[0]][road[1]] = road[2]
            dicts[road[1]][road[0]] = road[2]

        # process
        # dijkstra's algorithm
        # distances[x] - the shortest path from 0 to x
        distances = [-1] * n
        ...
        while heap:
            ...
            if distances[vertex] == -1:
                ...
                for nxt in dicts[vertex]:
                    if distances[nxt] == -1:
                        heappush(heap, (distance + dicts[vertex][nxt], nxt))
        # print(distances)

        # post-process
        MODULO = 10 ** 9 + 7
        cache = defaultdict(int)
        def reverse(target, shortest):

            if shortest != distances[target]:
                return 0
            if target == 0:
                return 1

            key = str(target) + "-" + str(shortest)
            if key in cache:
                return cache[key]

            total = 0
            for prev in dicts[target]:
                total += reverse(prev, shortest - dicts[target][prev])

            cache[key] = total
            return total

        ans = reverse(n - 1, distances[n - 1])
        return ans % MODULO
...
n = 7
roads = [[0,6,7],[0,1,1],[1,2,3],[1,3,3],[6,3,3],[3,5,1],[6,5,1],[2,5,1],[0,4,5],[4,6,2]]
