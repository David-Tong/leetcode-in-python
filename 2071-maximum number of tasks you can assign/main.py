class Solution(object):
    def maxTaskAssign(self, tasks, workers, pills, strength):
        ...
        # helper function
        # can assign target numbers of tasks
        def canAssign(target, pills):
            if target > N:
                return False
            from sortedcontainers import SortedList
            sorted_workers = SortedList(workers)
            idx = target - 1
            while idx >= 0:
                # if the most strength worker can handle the hardest task
                if sorted_workers[-1] >= tasks[idx]:
                    sorted_workers.pop()
                # otherwise, if a worker can handle it with a pill
                else:
                    if pills > 0:
                        idx2 = sorted_workers.bisect_left(tasks[idx] - strength)
                        if idx2 == len(sorted_workers):
                            return False
                        else:
                            sorted_workers.pop(idx2)
                            pills -= 1
                    else:
                        return False
                idx -= 1
        ...
        while left + 1 < right:
            middle = (left + right) // 2
            if canAssign(middle, pills):
                left = middle
            else:
                right = middle - 1
        if canAssign(right, pills):
            return right
        else:
            return left
