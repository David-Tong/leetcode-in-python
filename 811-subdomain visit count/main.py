class Trie(object):
    ...
    def insert(self, cpdomain):
        """
        :type word: str
        :rtype: None
        """
        count = int(cpdomain.split()[0])
        domains = cpdomain.split()[1].split(".")[::-1]

        curr = self
        for domain in domains:
            if domain not in curr.nodes:
                curr.nodes[domain] = Trie()
            curr = curr.nodes[domain]
            curr.value += count
...
class Solution(object):
    def subdomainVisits(self, cpdomains):
        ...
        # helper function
        # output
        def output(node, label):
            for child in node.nodes:
                if label != "":
                    child_label = ".".join([child, label])
                else:
                    child_label = child
                output(node.nodes[child], child_label)
            if label != "":
                ans.append("{} {}".format(node.value, label))
