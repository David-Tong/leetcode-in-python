    ...
    def countTrapezoids(self, points):
        ...
        for idx in range(L):
            for idx2 in range(idx + 1, L):
                ...
                slopes[slope].append((idx, idx2, intercept))

        # process
        # helper function
        def getKey(p, p2, p3, p4):
            pairs = set([p, p2, p3, p4])
            if len(pairs) < 4:
                return None
            return "-".join([str(_) for _ in sorted(pairs)])
        ...
        for slope in slopes:
            ...
            for idx in range(l):
                for idx2 in range(idx + 1, l):
                    p, p2, intercept = slopes[slope][idx]
                    p3, p4, intercept2 = slopes[slope][idx2]
                    if intercept != intercept2:
                        key = getKey(p, p2, p3, p4)
                        if key and key not in trapezoids:
                            trapezoids.add(key)
