class Solution(object):
    def findItinerary(self, tickets):
        ...
        itineraries = defaultdict()
        for ticket in tickets:
            departure, arrival = ticket[0], ticket[1]
            if departure not in itineraries:
                itineraries[departure] = defaultdict(int)
            itineraries[departure][arrival] += 1

        itinerary = list()
        ans = list()
        def doItinerary(departure, itinerary, itineraries, count):
            if count == N:
                for _ in itinerary:
                    ans.append(_)
                return True

            if departure in itineraries:
                for arrival in sorted(itineraries[departure].keys()):
                    if itineraries[departure][arrival] > 0:
                        itineraries[departure][arrival] -= 1
                        if doItinerary(arrival, itinerary + [arrival], itineraries, count + 1):
                            return True
                        itineraries[departure][arrival] += 1
            return False

        doItinerary("JFK", ["JFK"], itineraries, 0)
        return ans
