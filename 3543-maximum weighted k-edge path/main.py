class Solution(object):
    def maxWeight(self, n, edges, k, t):
        """
        :type n: int
        :type edges: List[List[int]]
        :type k: int
        :type t: int
        :rtype: int
        """
        # pre-process
        from collections import defaultdict
        paths = defaultdict(dict)
        for edge in edges:
            paths[edge[0]][edge[1]] = edge[2]

        # process
        from collections import defaultdict
        cache = defaultdict(int)
        def dfs(vertex, l, w):
            key = "{}-{}-{}".format(vertex, l, w)
            if key in cache:
                return cache[key]

            res = - 1
            if l == k:
                if w < t:
                    res = w
            else:
                for nxt in paths[vertex]:
                    res = max(res, dfs(nxt, l + 1, w + paths[vertex][nxt]))
            cache[key] = res
            return res

        ans = -1
        for vertex in range(n):
            ans = max(ans, dfs(vertex, 0, 0))
        return ans


n = 3
edges = [[0,1,1],[1,2,2]]
k = 2
t = 4

n = 3
edges = [[0,1,2],[0,2,3]]
k = 1
t = 3

n = 3
edges = [[0,1,6],[1,2,8]]
k = 1
t = 6

n = 20
edges = [[0,1,1],[0,2,1],[0,3,1],[0,4,1],[0,5,1],[0,6,1],[0,7,1],[0,8,1],[0,9,1],[0,10,1],[0,11,1],[0,12,1],[0,13,1],[0,14,1],[0,15,1],[0,16,1],[0,17,1],[0,18,1],[0,19,1],[1,2,1],[1,3,1],[1,4,1],[1,5,1],[1,6,1],[1,7,1],[1,8,1],[1,9,1],[1,10,1],[1,11,1],[1,12,1],[1,13,1],[1,14,1],[1,15,1],[1,16,1],[1,17,1],[1,18,1],[1,19,1],[2,3,1],[2,4,1],[2,5,1],[2,6,1],[2,7,1],[2,8,1],[2,9,1],[2,10,1],[2,11,1],[2,12,1],[2,13,1],[2,14,1],[2,15,1],[2,16,1],[2,17,1],[2,18,1],[2,19,1],[3,4,1],[3,5,1],[3,6,1],[3,7,1],[3,8,1],[3,9,1],[3,10,1],[3,11,1],[3,12,1],[3,13,1],[3,14,1],[3,15,1],[3,16,1],[3,17,1],[3,18,1],[3,19,1],[4,5,1],[4,6,1],[4,7,1],[4,8,1],[4,9,1],[4,10,1],[4,11,1],[4,12,1],[4,13,1],[4,14,1],[4,15,1],[4,16,1],[4,17,1],[4,18,1],[4,19,1],[5,6,1],[5,7,1],[5,8,1],[5,9,1],[5,10,1],[5,11,1],[5,12,1],[5,13,1],[5,14,1],[5,15,1],[5,16,1],[5,17,1],[5,18,1],[5,19,1],[6,7,1],[6,8,1],[6,9,1],[6,10,1],[6,11,1],[6,12,1],[6,13,1],[6,14,1],[6,15,1],[6,16,1],[6,17,1],[6,18,1],[6,19,1],[7,8,1],[7,9,1],[7,10,1],[7,11,1],[7,12,1],[7,13,1],[7,14,1],[7,15,1],[7,16,1],[7,17,1],[7,18,1],[7,19,1],[8,9,1],[8,10,1],[8,11,1],[8,12,1],[8,13,1],[8,14,1],[8,15,1],[8,16,1],[8,17,1],[8,18,1],[8,19,1],[9,10,1],[9,11,1],[9,12,1],[9,13,1],[9,14,1],[9,15,1],[9,16,1],[9,17,1],[9,18,1],[9,19,1],[10,11,1],[10,12,1],[10,13,1],[10,14,1],[10,15,1],[10,16,1],[10,17,1],[10,18,1],[10,19,1],[11,12,1],[11,13,1],[11,14,1],[11,15,1],[11,16,1],[11,17,1],[11,18,1],[11,19,1],[12,13,1],[12,14,1],[12,15,1],[12,16,1],[12,17,1],[12,18,1],[12,19,1],[13,14,1],[13,15,1],[13,16,1],[13,17,1],[13,18,1],[13,19,1],[14,15,1],[14,16,1],[14,17,1],[14,18,1],[14,19,1],[15,16,1],[15,17,1],[15,18,1],[15,19,1],[16,17,1],[16,18,1],[16,19,1],[17,18,1],[17,19,1],[18,19,1]]
k = 20
t = 600

solution = Solution()
print(solution.maxWeight(n, edges, k, t))
