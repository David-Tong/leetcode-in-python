class Solution(object):
    def maximumScore(self, nums, multipliers):
        ...
        # dp[x][y] - after x operations and select y numbers from the left
        ...
        for x in range(1, M + 1):
            for y in range(x + 1):
                if y > 0:
                    dp[x][y] = max(dp[x][y], dp[x - 1][y - 1] + nums[y - 1] * multipliers[x - 1])
                if x - y > 0:
                    dp[x][y] = max(dp[x][y], dp[x - 1][y] + nums[N - (x - y)] * multipliers[x - 1])
